...
if [ -z "$SOURCE_ACCOUNT" ]; then
    echo "❌ STELLAR_ADMIN_SECRET not set."
    echo "Set it in your environment with:"
    echo "    export STELLAR_ADMIN_SECRET=YOUR_SECRET_KEY"
    echo "Or add it to chainfund-backend/.env as:"
    echo "    STELLAR_ADMIN_SECRET=YOUR_SECRET_KEY"
    exit 1
fi
...
echo "2. Start the backend: cd chainfund-backend && python simple_server.py"
echo "3. Start the frontend: cd chainfund-frontend && npm run dev"
